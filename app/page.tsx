...
import { IcAlert, IcBarChart, IcCalendar, IcChevR } from "@/components/Ic";
...
export default function Home() {
  ...
  return (
    <main className="page" style={{ background:"#fff" }}>
      ...
      {/* HEADER */}
      <div style={{ padding:"56px 20px 0", display:"flex",
        ...
        <button onClick={() => router.push("/perfil")} style={{
          ...
          boxShadow:"0 4px 12px rgba(30,58,95,0.25)", cursor:"pointer",
        }}>{ME.avatar}</button>
      </div>
      ...
      <div style={{ padding:"24px 20px 0",
        ...
        {[
          ...
          <div key={k.label} style={{ background:"#f7f7f7", borderRadius:"16px", padding:"14px 12px" }}>
            <div style={{ marginBottom:"8px" }}>{k.icon}</div>
            <div style={{ fontSize:"26px", fontWeight:"800", color:"var(--text)",
              letterSpacing:"-1px" }}>{k.val}</div>
      ...
      {/* INGRESOS MINI CHART */}
      <div style={{ margin:"24px 20px 0", background:"#f7f7f7",
        borderRadius:"20px", padding:"18px" }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          alignItems:"center", marginBottom:"14px" }}>
          <div>
            <p style={{ fontSize:"12px", color:"var(--text2)" }}>Ingresos junio</p>
            <p style={{ fontSize:"22px", fontWeight:"800", color:"var(--text)",
              letterSpacing:"-0.5px" }}>
              $96,000 <span style={{ fontSize:"13px", color:"#10b981", fontWeight:600 }}>↑9%</span>
            </p>
          </div>
        ...
        <div style={{ display:"flex", alignItems:"flex-end", gap:"6px", height:"52px" }}>
          {KPI.map(k => (
            <div key={k.mes} style={{ flex:1, display:"flex",
      ...
      <div style={{ padding:"28px 20px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          ...
          <h2 style={{ fontSize:"18px", fontWeight:"800", color:"var(--text)",
            letterSpacing:"-0.3px" }}>Mis expedientes</h2>
          <button onClick={() => router.push("/clientes")} style={{
            background:"none", border:"none", color:"var(--accent)",
            fontSize:"13px", fontWeight:600, cursor:"pointer",
          }}>Ver todos</button>
        </div>

        {/* Carrusel con touch drag real */}
        <div style={{
          display:"flex", gap:"12px",
          overflowX:"auto",
          /* momentum scroll iOS */
          WebkitOverflowScrolling:"touch" as any,
          /* pan solo horizontal, no bloquea scroll vertical de página */
          touchAction:"pan-x",
          paddingBottom:"8px",
          ...
          {CLIENTES.slice(0,5).map(c => (
            <div key={c.id} className="pressable"
              ...
              <div style={{ padding:"10px 12px", background:"white" }}>
                ...
                <div style={{ height:"4px", background:"#f0f0f0", borderRadius:"4px" }}>
                  <div style={{ width:`${c.avance}%`, height:"100%",
                    background:c.color, borderRadius:"4px" }}/>
                </div>
              </div>
            </div>
          ))}
      ...
      {urgentes.length > 0 && (
        <div style={{ padding:"28px 20px 0" }}>
          <h2 style={{ fontSize:"18px", fontWeight:"800", color:"var(--text)",
            letterSpacing:"-0.3px", marginBottom:"14px" }}>Atención urgente</h2>
          {urgentes.map(t => {
            ...
            return (
              <div key={t.id} style={{
                ...
                border:"1px solid #fecaca", marginBottom:"8px", cursor:"pointer",
              }}
              onClick={() => router.push(`/clientes/${t.clienteId}`)}>
